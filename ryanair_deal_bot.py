...
def booking_link(from_code, to_code, out_date, in_date=None):
    """Best-effort Ryanair booking search link. For one-way this is solid; for
    round-trip it mirrors params with a 'tp' prefix based on Ryanair's own site
    pattern, but this specific part is unverified - check it actually pre-fills
    the return leg before trusting it blindly."""
    base = "https://www.ryanair.com/pl/en/trip/flights/select"
    is_return = bool(in_date)
    params = (
        ...
        f"&isReturn={'true' if is_return else 'false'}"
        f"&originIata={from_code}&destinationIata={to_code}"
    )
    if is_return:
        params += (
            f"&tpAdults=1&tpTeens=0&tpChildren=0&tpInfants=0"
            f"&tpStartDate={out_date}&tpEndDate={in_date}"
            f"&tpDiscount=0&tpPromoCode="
            f"&tpOriginIata={from_code}&tpDestinationIata={to_code}"
        )
    return base + params
...
def trip_length_keyboard(min_n, max_n):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("➖", callback_data="tl:max-"),
         ...
         InlineKeyboardButton("➕", callback_data="tl:max+")],
        [InlineKeyboardButton("Day trip (0-1)", callback_data="tl:preset:0:1"),
         InlineKeyboardButton("Weekend (2-3)", callback_data="tl:preset:2:3")],
        [InlineKeyboardButton("Week (6-8)", callback_data="tl:preset:6:8"),
         InlineKeyboardButton("2 weeks (13-15)", callback_data="tl:preset:13:15")],
...
async def handle_callback_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data.startswith("tl:preset:"):
        _, _, min_n, max_n = data.split(":")
        state["min_nights"], state["max_nights"] = int(min_n), int(max_n)
        save_state(state)
        await query.answer()
        await query.edit_message_reply_markup(reply_markup=trip_length_keyboard(int(min_n), int(max_n)))
