...
DEFAULT_STATE = {
    ...
    "seen_deals": [],        # Ryanair one-way, keyed by dest-date
    "seen_round_trips": [],  # Ryanair round-trip, keyed by dest-outdate-indate
    "seen_lot": [],          # LOT deals via Kiwi, keyed by dest-date
    "seen_wizzair": [],      # Wizzair deals via Kiwi, keyed by dest-date
    "last_check": None,
}
...
async def check_for_deals(context: ContextTypes.DEFAULT_TYPE):
    ...
    if state["track_oneway"]:
        try:
            ...
            for d in new_oneway:
                ...
                lines.append(
                    f"✈️ *Ryanair one-way:* {d['from']} → {d['to']} on {d['date'][:10]}: "
    ...
    if state["track_roundtrip"]:
        try:
            ...
            for d in new_roundtrip:
                ...
                lines.append(
                    f"🔁 *Ryanair round trip:* {d['from']} → {d['to']}\n"
    ...
    if KIWI_API_KEY:
        try:
            ...
            new_lot = [d for d in lot_deals if kiwi_key(d) not in state["seen_lot"]]
            for d in new_lot:
                link = d["link"] or "https://www.lot.com/"
                lines.append(
                    f"🛫 *LOT one-way:* {d['from']} → {d['to']} on {d['date'][:10]}: "
                    f"*{d['price']:.2f} {d['currency']}*\n[Book →]({link})"
                )
                state["seen_lot"].append(kiwi_key(d))
        except Exception as e:
        ...
        try:
            ...
            new_wizz = [d for d in wizz_deals if kiwi_key(d) not in state["seen_wizzair"]]
            for d in new_wizz:
                link = d["link"] or "https://wizzair.com/"
                lines.append(
                    f"🟣 *Wizzair one-way:* {d['from']} → {d['to']} on {d['date'][:10]}: "
                    f"*{d['price']:.2f} {d['currency']}*\n[Book →]({link})"
                )
                state["seen_wizzair"].append(kiwi_key(d))
        except Exception as e:
            log.error(f"Wizzair fetch failed: {e}")

    if lines:
        header = "🎉 *New flight deals!*\n\n"
        batch = header
        batches = []
        for line in lines:
    ...
    state["seen_lot"] = state["seen_lot"][-500:]
    state["seen_wizzair"] = state["seen_wizzair"][-500:]
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "📊 Status · 🔍 Check now · 💶 One-way price · 🔁 Round-trip price · 🔀 Trip types · 🕐 Times of day\n\n"
        "*Typed commands:*\n"
        ...
        "unless you want a specific window.",
        parse_mode="Markdown",
    )
