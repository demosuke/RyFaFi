...
# ---------------------------------------------------------------------------
def fetch_hotels(city_query, checkin_date, checkout_date, adults, children, extra_terms="", debug=False):
    """Searches hotels for a city/date range via SerpApi. checkin/checkout must be
    YYYY-MM-DD. Returns up to 5 highly-rated results sorted by rating then price."""
    if not SERPAPI_KEY:
        return []
    ...
    for item in data.get("properties", []):
        try:
            ...
            link = item.get("link", "")
            results.append({"name": name, "rate": rate, "rating": rating, "link": link})
        except (KeyError, TypeError, AttributeError):
            continue

    # prefer highly-rated (8+) results, then sort by rating desc within that
    highly_rated = [r for r in results if isinstance(r["rating"], (int, float)) and r["rating"] >= 8]
    pool = highly_rated if highly_rated else results
    pool.sort(key=lambda r: (-(r["rating"] or 0)))
    return pool[:5]
...
def hotel_pets_keyboard(n, dog, kid, city, checkin, checkout):
    dogkid = f"{1 if dog else 0}{1 if kid else 0}"
    dog_mark = "✅ " if dog else ""
    kid_mark = "✅ " if kid else ""
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"{dog_mark}🐕 Dog", callback_data=f"hpet:dog:{n}:{dogkid}:{city}:{checkin}:{checkout}")],
        [InlineKeyboardButton(f"{kid_mark}🧒 Small kid", callback_data=f"hpet:kid:{n}:{dogkid}:{city}:{checkin}:{checkout}")],
        [InlineKeyboardButton("Search 🔍", callback_data=f"hpet:done:{n}:{dogkid}:{city}:{checkin}:{checkout}")],
    ])
...
async def handle_callback_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("setmax:"):
        ...
        parts = data.split(":")
        action, n, dogkid, city, checkin, checkout = parts[1], parts[2], parts[3], parts[4], parts[5], parts[6]
        dog, kid = dogkid[0] == "1", dogkid[1] == "1"

        if action in ("dog", "kid"):
            if action == "dog":
                dog = not dog
            else:
                kid = not kid
            await query.answer()
            await query.edit_message_reply_markup(reply_markup=hotel_pets_keyboard(int(n), dog, kid, city, checkin, checkout))
            return
        ...
        try:
            hotels = fetch_hotels(city, checkin, checkout, int(n), 1 if kid else 0, extras)
        except Exception as e:
