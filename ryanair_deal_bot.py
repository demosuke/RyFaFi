...
def fetch_hotels(city_query, checkin_date, checkout_date, adults, children, extra_terms="", debug=False):
    """Searches hotels and apartments for a city/date range via SerpApi. checkin/checkout
    must be YYYY-MM-DD. Returns up to 5 highly-rated results, cheapest first."""
    if not SERPAPI_KEY:
        return []

    q = f"{city_query} hotels and apartments"
    if extra_terms:
        q += f" {extra_terms}"
    ...
    for item in data.get("properties", []):
        try:
            ...
            link = item.get("link", "")
            results.append({"name": name, "rate": rate, "price": parse_price(rate), "rating": rating, "link": link})
